#!/usr/bin/env python
"""Closed-loop GR00T evaluation for a RoboTwin task, using RoboTwin's native
world-frame, absolute endpose representation (no camera-frame reprojection, no
rot6d) -- the schema script/lerobot/convert_robotwin_to_lerobot.py (this repo)
...
that already has gr00t (editable) + its dependencies installed:
    ...
    python script/eval_gr00t_endpose.py --model-path /path/to/checkpoint --task stack_bowls_three \\
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", default=None, help="Defaults to eval_result/<task>/GR00T-Endpose/<task_config>/<timestamp>"
    )
    parser.add_argument(
        "--gr00t-path",
...
def main():
    ...
    output_dir = (
        Path(cli.output_dir)
        if cli.output_dir
        # "GR00T-Endpose" is just this script's own output-path convention (distinct from
        # eval_gr00t.py's "GR00T" segment) -- rename freely, nothing downstream depends on it.
        else Path(f"eval_result/{cli.task}/GR00T-Endpose/{cli.task_config}/{current_time}")
    )
